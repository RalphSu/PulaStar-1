...
- ( NSMutableDictionary *)sendPay_activity: (NSString *)title price:(NSString *)price buyItemNo:(NSString *)buyItemNo buyItemNum:(NSString*)buyItemNum
{
    ...
    // 把时间戳转成字符串
    NSString *currentTimeStr = [myFormatter stringFromDate:myDate];
    
    // 打印的字符串为当前的时间
    NSLog(@"curremtTimeStr is ------%@", currentTimeStr);
    ...
    [packageParams setObject: @"196.168.1.1"    forKey:@"spbill_create_ip"];//发器支付的机器ip
    [packageParams setObject: payprice             forKey:@"total_fee"];       //订单金额，单位为分
    ...
    if ( prePayid != nil) {
        ...
        NSMutableDictionary *signParams = [NSMutableDictionary dictionary];
        [signParams setObject: appid        forKey:@"appid"];
        //[signParams setObject: attach       forKey:@"attach"];
        [signParams setObject: nonce_str    forKey:@"noncestr"];
        [signParams setObject: package      forKey:@"package"];
        [signParams setObject: mchid        forKey:@"partnerid"];
        [signParams setObject: prePayid     forKey:@"prepayid"];
